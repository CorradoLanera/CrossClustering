#' CrossClustering: a clustering algorithm with automatic estimation of the number of clusters and identification of outliers
#'
#' This function performs the CrossClustering algorithm. This method combines the ward and complete linkage algorighms, providing automatic estimation of a suitable number of clusters and identification of outlier elements.
#'
...
#' 
#' ### Run CrossClustering
#' CrossClustering(d, k.w.min=2, k.w.max=5, k.c.max=6, out=TRUE)
#'  
#'  @author
...
CrossClustering<-function(d,k.w.min,k.w.max,k.c.max,out=TRUE)
{
  ...
  if(is.null(dim(k.star))){
    ...
    cluster.list <- apply(k.star, 1, max.proportion.function, beta.clu.ward=beta.clu.ward, beta.clu.complete=beta.clu.complete, return.list=T)
