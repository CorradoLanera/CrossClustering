...
test_that("correct output class", {
  expect_type(
    cc_max_proportion(k, beta.clu.ward, beta.clu.method2),
    'integer'
  )
  expect_type(
    cc_max_proportion(k, beta.clu.ward, beta.clu.method2,
...
test_that("correct known result", {
  expected <- 6L
  actual   <- cc_max_proportion(k, beta.clu.ward, beta.clu.method2)

  actual_list <- cc_max_proportion(k, beta.clu.ward, beta.clu.method2,
    return.list = TRUE
  )
